...
export function registerTier1(connections: ConnectionManager): vscode.Disposable[] {
  ...
  return [
    ...
    reg('sliver.session.reconfig', async (node?: Node) => {
      const client = active();
      const session = await resolveSession(connections, node);
      if (!client || !session) {
        return;
      ...
      const cur = Math.round((Number(session.ReconnectInterval) || 0) / NS) || 60;
      const reconnect = await vscode.window.showInputBox({
        prompt: 'Reconnect interval (seconds) — how long the implant waits before retrying a dropped C2',
        value: String(cur),
        validateInput: (v) => (/^\d+$/.test(v.trim()) && Number(v) > 0 ? undefined : 'Enter a positive whole number of seconds'),
      });
      ...
      try {
        await client.reconfigure({
          ReconnectInterval: String(Number(reconnect) * NS),
          Request: { SessionID: session.ID },
        });
        void vscode.window.showInformationMessage(`Reconfigured ${session.Name}: reconnect ${reconnect}s.`);
        void vscode.commands.executeCommand('sliver.refresh');
      } catch (err) {
        void vscode.window.showErrorMessage(`Reconfigure failed: ${friendlyError(err)}`);
      }
    }),

    reg('sliver.beacon.reconfig', async (node?: Node) => {
      const client = active();
      if (!client || !node?.beacon) {
        return;
      }
      const NS = 1_000_000_000;
      const curInterval = Math.round((Number(node.beacon.Interval) || 0) / NS) || 60;
      const curJitter = Math.round((Number(node.beacon.Jitter) || 0) / NS) || 30;
      const interval = await vscode.window.showInputBox({
        prompt: 'Beacon interval (seconds) — how often the beacon checks in',
      ...
      const jitter = await vscode.window.showInputBox({
        prompt: 'Beacon jitter (seconds) — random delay added to each check-in',
        value: String(curJitter),
        validateInput: (v) => (/^\d+$/.test(v.trim()) ? undefined : 'Enter a whole number of seconds (0 for none)'),
      });
      if (jitter === undefined) {
        return;
      }
      try {
        // Beacon reconfig is queued as a task — must be async, addressed by BeaconID.
        await client.reconfigure({
          BeaconInterval: String(Number(interval) * NS),
          BeaconJitter: String(Number(jitter) * NS),
          Request: { BeaconID: node.beacon.ID, Async: true },
        });
        void vscode.window.showInformationMessage(
          `Queued reconfig for ${node.beacon.Name}: interval ${interval}s, jitter ${jitter}s (applies on next check-in).`,
        );
        void vscode.commands.executeCommand('sliver.refresh');
      } catch (err) {
        void vscode.window.showErrorMessage(`Reconfigure failed: ${friendlyError(err)}`);
