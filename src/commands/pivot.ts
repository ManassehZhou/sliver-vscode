...
export function registerPivot(connections: ConnectionManager): vscode.Disposable[] {
  ...
  return [
    ...
    reg('sliver.session.wgPortfwdStop', async (node?: Node) => {
      const client = requireActive(connections);
      const s = client && (await resolveSession(connections, node));
      if (!client || !s) {
        return;
      ...
      try {
        const r = await client.wgListForwarders(s.ID);
        const items: { label: string; id: number }[] = (r.Forwarders ?? []).map((f: any) => ({
          label: `#${f.ID}  ${f.LocalAddr} → ${f.RemoteAddr}`,
          id: f.ID,
        }));
        if (!items.length) {
          void vscode.window.showInformationMessage('No active WG port-forwards on this session.');
          return;
        }
        const pick = await vscode.window.showQuickPick(items, { placeHolder: 'Stop which WG port-forward?' });
        if (!pick) {
          return;
        }
        await client.wgStopPortForward({ ID: pick.id, Request: { SessionID: s.ID } });
        void vscode.window.showInformationMessage('WG port-forward stopped.');
      } catch (err) {
        void vscode.window.showErrorMessage(`WG portfwd stop failed: ${friendlyError(err)}`);
      }
    }),

    reg('sliver.session.wgSocksStop', async (node?: Node) => {
      const client = requireActive(connections);
      const s = client && (await resolveSession(connections, node));
      if (!client || !s) {
        return;
      ...
      try {
        const r = await client.wgListSocksServers(s.ID);
        const items: { label: string; id: number }[] = (r.Servers ?? []).map((sx: any) => ({
          label: `#${sx.ID}  ${sx.LocalAddr}`,
          id: sx.ID,
        }));
        if (!items.length) {
          void vscode.window.showInformationMessage('No active WG SOCKS proxies on this session.');
          return;
        }
        const pick = await vscode.window.showQuickPick(items, { placeHolder: 'Stop which WG SOCKS proxy?' });
        if (!pick) {
          return;
        }
        await client.wgStopSocks({ ID: pick.id, Request: { SessionID: s.ID } });
        void vscode.window.showInformationMessage('WG SOCKS proxy stopped.');
      } catch (err) {
        void vscode.window.showErrorMessage(`WG socks stop failed: ${friendlyError(err)}`);
      }
    }),
