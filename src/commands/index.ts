...
export function registerCommands(
  ...
  return [
    ...
    reg('sliver.listener.start', async () => {
      const client = requireActive();
      if (!client) {
        return;
      }
      const type = await vscode.window.showQuickPick(['mtls', 'http', 'https', 'dns', 'wg', 'stager (tcp)'], {
        placeHolder: 'Listener type',
      });
      if (!type) {
        return;
      }
      const host = (await vscode.window.showInputBox({ prompt: 'Bind host', value: '0.0.0.0' })) ?? '0.0.0.0';
      const defaultPort = type === 'http' ? '80' : type === 'https' ? '443' : type === 'dns' ? '53' : type === 'wg' ? '53' : '8888';
      const portStr = await vscode.window.showInputBox({ prompt: 'Port', value: defaultPort });
      ...
      try {
        if (type === 'mtls') {
          await client.startMtlsListener({ Host: host, Port: port });
        } else if (type === 'http') {
          await client.startHttpListener({ Host: host, Port: port, Secure: false });
        } else if (type === 'https') {
          await client.startHttpsListener({ Host: host, Port: port, Secure: true });
        } else if (type === 'wg') {
          await client.startWgListener({ Host: host, Port: port, TunIP: '100.64.0.1', NPort: 8888, KeyPort: 1337 });
        } else if (type === 'stager (tcp)') {
          await client.startTcpStagerListener({ Protocol: 0, Host: host, Port: port });
        } else {
          const domains = await vscode.window.showInputBox({ prompt: 'DNS domains (comma-separated, with trailing dot)' });
          await client.startDnsListener({
            Host: host,
            Port: port,
            Domains: (domains ?? '').split(',').map((d) => d.trim()).filter(Boolean),
          });
        }
        void vscode.window.showInformationMessage(`${type.toUpperCase()} listener started on ${host}:${port}`);
        trees.jobs.refresh();
      } catch (err) {
